...
class AGVideoPlayerView: UIView {
    ...
    var previewImageUrl: URL? {
        didSet {
            previewImageView.loadImg(str: previewImageUrl!.absoluteString)
...
extension AGVideoPlayerView {
    ...
    private func configurateControls() {
        ...
        playIcon = UIImageView(image: UIImage(named:"mic_video_play"))
        ...
        self.insertSubview(previewImageView, belowSubview: playerController.view)
...
extension AGVideoPlayerView {
    fileprivate func isVisible() -> Bool {
        ...
        let selfFrame = self.convert(self.bounds, to: keyWindow)
...
//        videoAsset?.loadValuesAsynchronously(forKeys: [], completionHandler: { [weak self] in
//            DispatchQueue.main.async {
//                if let playing = self?.isPlaying, playing { return }
////                if self?.isPlaying == true {
//                    self?.playIcon.isHidden = true
//                    if self?.playerController.player?.status == AVPlayer.Status.readyToPlay{
...
extension AGVideoPlayerView {
    ...
    fileprivate func removePlayerObservers() {
        do{
            ...
            //            self.playerController.contentOverlayView?.removeObserver(self, forKeyPath: "bounds")
        }
        catch(_){
