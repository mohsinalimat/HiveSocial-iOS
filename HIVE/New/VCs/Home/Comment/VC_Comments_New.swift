...
class VC_Comments_New: UIViewController {
    //MARK: - variables
    ...
    func trackUserBlock(){
        if blockTracker != nil || self.post == nil || self.post.ouid == Me.uid{
            return
        }
    ...
    func setupTxtInput(){
        ...
        vAccessory.opReplyAction = {
            ...
            cmt.sendComment(media: self.selectedMedia, gif: self.selectedGif, txt: self.txt_input.text) { (res, str) in
    ...
    func setupCommentsView(){
        ...
        v_comments.provider = ComposedProvider(sections: [
            BasicProvider(
                dataSource: [post],
                viewSource: ClosureViewSource(viewGenerator: { (post: Post, index: Int) -> cv_comment_post_header in
                    let v = Bundle.main.loadNibNamed("cv_comment_post_header", owner: self, options: nil)?[0] as! cv_comment_post_header
                    return v
                }, viewUpdater: { (v: cv_comment_post_header, post: Post, index: Int) in
                    v.setPost(post: post)
                    v.opMoreAction = {
                    }
                ...
                sizeSource: { (index: Int, post: Post, size: CGSize) -> CGSize in
                    return CGSize(width: size.width, height: Utils.getHeightCmtPostHeader(post: post))
                }
            ),
            BasicProvider(
                ...
                viewSource: ClosureViewSource(viewGenerator: { (cmt: Comment, index: Int) -> cv_comment_item in
                    ...
                    v.setComment(cmt: cmt, myPost: self.post.ouid == Me.uid, showLabel: self.expended[cmt.mid] ?? false)
                    if cmt.type == .IMAGE{
                    ...
                    v.opShowRepliesAction = { showComments in
                        if showComments{
                            ...
                            FPOSTS_REF
                                .document(cmt.pid.isEmpty ? self.post.pid : cmt.pid)
                                .collection(cmt.mid)
                ...
                sizeSource: { (index: Int, cmt: Comment, size: CGSize) -> CGSize in
                    return CGSize(width: size.width, height: Utils.getHeightCmtItem(cmt: cmt))
                }
            )
        ])
    }
    ...
    func loadData(){
        ...
        cmtListener = FPOSTS_REF
            .document(post.pid)
            .collection(Post.key_collection_comments)
            .order(by: Comment.key_created, descending: true)
            .limit(to: 20)
    ...
    func loadMoreData(){
        ...
        FPOSTS_REF
            ...
            .getDocuments { (doc, err) in
                ...
                self.isLoadingMore = false
                print("loaded more")
//                self.commentsDataSource.data = self.comments.values
//                    .filter{
//                        !(MyBlocks[$0.uid] == true)
//                    }
//                    .sorted(by: { (cmt1, cmt2) -> Bool in
//                        cmt1.created > cmt2.created
//                    })
            }
    }
...
extension VC_Comments_New: GIPHYMediaManagerDelegate {
...
extension VC_Comments_New: AutocompleteManagerDelegate, AutocompleteManagerDataSource {
...
extension VC_Comments_New: UIScrollViewDelegate{
